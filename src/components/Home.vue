<template>
  <div class="container-fluid app__container" :class="{ 'oscuro' : this.UI === 'UI Clara' }">
    <app-header :total="total" 
                :totalIngresos="totalIngresos" 
                :totalGastos="totalGastos" 
                :porcentajeTotal="porcentajeTotal" 
                :mostrarAdvertencia="advertencia"
                >
                <!-- <a @click="cambiarAuth">Cerrar sesión</a> -->
    </app-header>
    <app-inputs-bar @addedItem="nuevoItem" :newItem="newItem" 
                    @mostrarAdvertencia="mostrar"            
                    >
    </app-inputs-bar>
    <div class="row lists__container">
      <app-ingresos-list :ingresos="ingresos"
                        @itemBorrado="borrarIngreso"
                        >
      </app-ingresos-list>
      <app-gastos-list  :gastos="gastos" 
                        :calcularPorcentajeItem="calcularPorcentajeItem" 
                        :totalGastos="totalGastos"
                        @itemBorrado="borrarGasto"
                        >
      </app-gastos-list>
    </div>
    <!-- <pre>{{ $data }}</pre> -->
    <!-- Switch Cambio UI -->
    <label class="switch">
        <span class="switch-texto">{{ UI }}</span>
        <input type="checkbox">
        <span class="slider round" @click="cambiarInterfaz"></span>
    </label>
  </div>
</template>
